...
class AcademicSettings extends PluginSettingTab {
    ...
    renderAppearance(container: HTMLElement) {
        ...
        for (const [field, name, fallback] of colors) {
            ...
            row.addToggle(c => { c.toggleEl.dataset.anControl = field; c.setValue(!!entry[field]).onChange(async enabled => { await this.changeAppearance(field, enabled ? fallback : ''); redraw(); }); });
        ...
        new Setting(root).setName(t('恢复此环境默认外观')).addButton(b => { b.buttonEl.dataset.anControl = 'reset'; b.setButtonText(t('恢复默认')).onClick(async () => {
            ...
            this.plugin.settings.customAppearance = JSON.stringify(all); await this.plugin.saveSettings(); redraw();
        }); });
