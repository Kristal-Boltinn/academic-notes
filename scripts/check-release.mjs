...
assert.equal(manifest.id, 'academic-notes'); assert.equal(manifest.isDesktopOnly, false);
...
assert.ok(!/require\(["']node:timers["']\)/.test(bundle), 'Release must not require Node timers on mobile');
